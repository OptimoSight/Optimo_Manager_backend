...
class SubscriptionCreate(BaseModel):
    ...
    features: Optional[dict] = None  # Explicitly optional
...
async def get_subscriptions(
    ...
    try:
        logger.info(f"Fetching subscriptions for user {user.email}")
        role = db.query(Role).filter(Role.id == user.role_id).first()
        if not role:
            logger.error(f"Role not found for user {user.email} with role_id {user.role_id}")
            raise HTTPException(status_code=500, detail="User role not found")
        if role.role == "guest":
            logger.warning(f"Guest {user.email} attempted to view subscriptions")
            raise HTTPException(status_code=403, detail="Guests cannot view subscriptions")
